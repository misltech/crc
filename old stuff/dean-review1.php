...
// Set return path if user wants to edit information

$_SESSION['edit_return'] = "dean-review1.php?id=".$searchKey;

// Do not change page_key value, it will break the system

$_SESSION['page_key'] = "dean1";
...
if ($result->num_rows > 0 ) {
    ...
    $_SESSION['dept'] = $dept_code = $result['dept_code'];
    
    $sql_deptInfo = "SELECT chair_email from s20_academic_dept_info where dept_code='$dept_code'";

    $result_dept = $conn->query($sql_deptInfo);

    if ($result_dept->num_rows > 0) {
        $result_dept = $result_dept->fetch_assoc();
        $chair_email = $result_dept['chair_email'];
    }
...
<h1>Review Application Info:</h2>
    ...
    <hr>
    <form method="post" action="backend/dean-appinfo1.php">

        <h3>Course Information:</h3>

        <input type="hidden" name="f_name" value="<?php echo $result_student['student_first_name']; ?>">
        <input type="hidden" name="l_name" value="<?php echo $result_student['student_last_name']; ?>">

        <input type="hidden" name="dept" value="<?php echo $result['dept_code']; ?>">
        <input type="hidden" name="title" value="<?php echo $result['name']; ?>">
        <input type="hidden" name="chair_email" value="<?php echo $chair_email; ?>">
...
// Show edit button if permission is allowed
        ...
        if ($courseInfo == '1') {
?>
            <button id="edit_courseInfo" type="button" onclick="window.location.href='/p/f18-02/f19-v1/student-application1.php?id=<?php echo $searchKey ?>'" class="btn btn-primary">Edit Course Information</button>
<?php
        }
?>
...
<?php
        ...
        $sql_empInfo = "SELECT * from s20_employer_info where employer_email='$empEmail'";
