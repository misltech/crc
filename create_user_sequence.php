...
<div class="modal-header">
    <h1>Create Workflow Order</h1>
	
</div>
<form action="backend/admin/set-user-sequence.php" method="post">
    <?php
        echo "<select id='deptselect' name='dept' onchange=loadSequence(this.value)>";
            echo "<option selected disabled hidden>Select a Department</option>"; 
                    include("backend/loadDepartments.php");
			echo "</select>";
			echo "<input id='coursenum' type='text' name='coursenum' placeholder='Enter Course Number' required></input>";
//        include("../../backend/loadDepartments.php");
    ?>
	<h3>
		To create Workflow Order <b>click and drag orange buttons from the left side to the right side</b>.<br>
